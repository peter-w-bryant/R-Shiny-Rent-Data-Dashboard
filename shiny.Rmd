...
california <- st_read("https://uwmadison.box.com/shared/static/iz2vor3rma5rzacvul5vbyfekbcnmcty.geojson") %>%
  dplyr::select(-AdminRegion, -FireMAR, -LawMAR, -State_FIPS_ID, -County_FIPS_ID, -Shape__Area, -Shape__Length) %>% 
  rename(County = CountyName)

california_df <- as.data.frame(california)
california_merged <- merge(california_df, california_rent_data_2021, by = "County")

california_rent_data <- left_join(california_rent_data, california %>% dplyr::select(County, OBJECTID),
                                   by = c("County"))

california <- st_as_sf(california_merged) %>% 
...
cali_leaflet_plot <- function(bedrooms) {
  ...
  california[[bedrooms]] <- california[[bedrooms]]

  # Create a color palette based on the numeric values
  color_palette <- colorNumeric(palette = "viridis", domain = california[[bedrooms]])

  map <- leaflet(california) %>%
    addTiles() %>%
    addPolygons(
      fillColor = ~color_palette(california[[bedrooms]]),
      color = "black",
      weight = 1,
      opacity = 1,
      fillOpacity = 0.5,
  ...
  map %>% addLegend(
    ...
    title = bedrooms,
    pal = color_palette,
    values = ~as.numeric(california[[bedrooms]]),
    opacity = 1
  )
}

#cali_leaflet_plot("0BR")
```
```{r}
...
# Example usage
#create_bedrooms_line_plot("San Francisco", "2BR")
```
