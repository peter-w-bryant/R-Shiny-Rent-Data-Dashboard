...
head(us_counties_geojson)

state_leaflet_plot <- function() {
  color_palette <- colorNumeric(palette = "viridis", domain = us_counties_geojson[["0BR"]])

  # Convert the character data to numeric
  map <- leaflet(data = us_counties_geojson) %>%
    addTiles() %>%
    addPolygons(
      fillColor = ~color_palette(us_counties_geojson[["0BR"]]),
      color = "black",
      weight = 1,
      opacity = 1,
      fillOpacity = 0.5,
  ...
  tabPanel(
    ...
    p("This interactive visualization allows you to analyze key factors and assist you to make informed, low-risk investments. To start, click a variable of interest from the dropdown menu. The plot below maps your variable of interest on a US map, and if you click on a state the value the state's color is encoding will be presented. The median house prices and median days on the market will also display supplemental plots to the right."
     ...
     fluidRow(
      ...
      column(width = 8, leafletOutput("california_map", height = "600px")),
      column(
        width = 4,
...
,
     plotOutput("bedrooms_line_plot", width = "100%", height = "300px"),
    )
  ),
  # New York Analysis Tab+Page
    tabPanel(
      "New York",
      mainPanel(
...
server <- function(input, output, session) {
  ...
  # CALIFORNIA ANALYSIS
  output$california_map <- renderLeaflet({cali_leaflet_plot(input$bedrooms)})
  
  selected_county <- reactiveVal()

  observeEvent(input$california_map_shape_click, {
    click_data <- input$california_map_shape_click
  ...
  output$county_info <- renderDT({
    ...
    county_data <- california_rent_data[california_rent_data$OBJECTID == selected_county(), ] %>% 
  ...
  output$bedrooms_line_plot <- renderPlot({
    county_data <- california_rent_data[california_rent_data$OBJECTID == selected_county(), ] %>% 
      pull(County)
    create_bedrooms_line_plot(county_data, input$bedrooms)
  })
