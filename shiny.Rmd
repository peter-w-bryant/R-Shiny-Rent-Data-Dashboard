---
title: "STAT 436 Milestone #3 Submission (2023)"
author: "Peter Bryant, Andreea Ghenciu, Amber Olson, Tapasya Nangpal, Christina Yao"
---
...
```{r}
# Extract 2021 Rental Data
california_rent_data <- read_csv("https://uwmadison.box.com/shared/static/u9nt6aafkrf1vvcknnkjckwsy7qnrvqo.csv")
california_rent_data_2021 <-california_rent_data %>% filter(Year == 2021)

# Clean up California rent data
california_rent_data = california_rent_data %>% 
  select(Year, County, `0BR`, `1BR`, `2BR`, `3BR`, `4BR`) %>% 
...
# Turn California geojson to DF for MERGE
california_df <- as.data.frame(california_geojson)
california_merged <- merge(california_df, california_rent_data_2021, by = "County")

california_merged

# Contains all rent data (not just 2021)
california_rent_data <- left_join(california_rent_data, california_geojson %>% dplyr::select(County, OBJECTID),
                                   by = c("County"))
# Contains California geojson + 2021 Rent Costs per County
california_geojson <- st_as_sf(california_merged) %>% 
  mutate(`0BR` = as.numeric(`0BR`))
```
...
# Read in csv file of all state rental data
all_rental_data <- read_csv("scrape/rentdata.csv") %>% 
  mutate_at(vars(`0BR`: `4BR`), as.numeric) %>% 
  # drop_na() %>% 
...
all_rental_data
all_rental_data_2021 = all_rental_data %>%
  filter(Year == 2021)
# 
# us_counties %>% 
...
us_counties_merged <- merge(us_counties_df, all_rental_data_2021, 
                            by.x = c("STATE_NAME", "COUNTY_NAME"), 
                            by.y = c("STATE_NAME", "COUNTY_NAME"))


us_counties_merged2 <- merge(us_counties_df, all_rental_data, 
                            by.x = c("STATE_NAME", "COUNTY_NAME"), 
                            by.y = c("STATE_NAME", "COUNTY_NAME"))
...
head(us_counties_merged)

# Contains California geojson + 2021 Rent Costs per County
us_counties_geojson <- st_as_sf(us_counties_merged)

county_info <- us_counties_merged[us_counties_merged$OBJECTID == '0500000US56037', c("COUNTY_NAME", "STATE_NAME")]

county_info[1]
#state_leaflet_plot("1BR")


county_data <- us_counties_merged2 %>%
  filter(COUNTY_NAME == "Elko" & STATE_NAME == "Nevada")

county_data
# county_data <- us_counties_merged2 %>%
#     filter(COUNTY_NAME == county_state_name[1] & STATE_NAME == county_state_name[2])
...
bedrooms = "0BR"
  ...
  plot <- ggplot(county_data, aes(x = Year, y = !!sym(bedrooms))) +
    ...
    ggtitle(paste0("Rent Prices for ", "Elko", " County"))
...
median_listing_days <- realty_data %>% 
  ...
  mutate(median_days_on_market_scaled = normalize(median_days_on_market) * -1) # mult by reciprocal (lower is better)
```
  ...
  tabPanel(
    ...
    p("This interactive visualization allows you to analyze key factors and assist you to make informed, low-risk investments. To start, click a variable of interest from the dropdown menu. The plot below maps your variable of interest on a US map, and if you click on a state the value the state's color is encoding will be presented. The median house prices and median days on the market will also display supplemental plots to the right."
     ...
     fluidRow(
      ...
      column(width = 8, leafletOutput("us_county_map", height = "600px")),
      column(
        width = 4,
        div(DTOutput("county_info"), id = "custom-datatable"),
          tags$style(HTML("
            #custom-datatable_wrapper .dataTables_length label,
            #custom-datatable_wrapper .dataTables_length select,
            #custom-datatable_wrapper .dataTables_filter label,
            ...
            #custom-datatable_wrapper .dataTables_length select option {
            ...
            #custom-datatable div.dataTables_wrapper .dataTables_length label {
              color: black;
            }
            #custom-datatable div.dataTables_wrapper .dataTables_paginate .paginate_button {
              background-color: #f5f5f5;
              color: black;
            }
          "))
...
,
    conditionalPanel(condition = "$('#selected_state_county_header').text() !== 'Showing All U.S. Counties'", plotOutput("bedrooms_line_plot", width = "100%", height = "500px")),
    )
  ),
...
server <- function(input, output, session) {
  ...
  weighted_sum_data <- reactive({
    education_weighted_data <- education_spending_ppcs %>%
      mutate(education_weighted = as.numeric(scaled_ppc) * input$education_spending_weight) %>% 
      select(`State`, education_weighted)
    
    hospital_weighted_data <- hospitals %>%
      mutate(hospital_weighted = as.numeric(scaled_count) * input$citizens_per_hospital_weight) %>% 
      select(`State`, hospital_weighted)

    median_home_prices_weighted_data <- median_listing_price %>%
      mutate(median_home_prices_weighted = as.numeric(median_listing_price_scaled) * input$median_home_prices_weight) %>% 
      select(`State`, median_home_prices_weighted)
    
    median_listing_days_weighted_data <- median_listing_days %>%
      mutate(median_listing_days_weighted = as.numeric(median_days_on_market_scaled) * input$median_listing_days_weight) %>% 
  ...
  # U.S. County ANALYSIS
  output$us_county_map <- renderLeaflet({state_leaflet_plot(input$bedrooms)})
  
  selected_county <- reactiveVal()

  observeEvent(input$us_county_map_shape_click, {
    click_data <- input$us_county_map_shape_click
    selected_county(click_data$id)
  })
  
  selected_county_info <- reactive({
    if (!is.null(selected_county())) {
      county_info <- us_counties_merged[us_counties_merged$OBJECTID == selected_county(), c("COUNTY_NAME", "STATE_NAME")]
      return(county_info)
    }
  })
  ...
  output$county_info <- renderDT({
    ...
    county_data <- us_counties_merged2[us_counties_merged2$OBJECTID == selected_county(), ] %>% 
  ...
  output$bedrooms_line_plot <- renderPlot({
    if (!is.null(selected_county())) {
  ...
  output$selected_state_county_header <- renderUI({
    if (!is.null(selected_county())) {
      tags$h3(paste(selected_county_info()[1], ",", selected_county_info()[2], collapse = ""), br())
    } else {
