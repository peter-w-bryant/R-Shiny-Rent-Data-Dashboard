...
create_map_plot <- function(map_data, fill_var, title, low_is_good = TRUE) {
  ...
  ggplot(data=map_data, aes(x = long, y = lat, group = group, fill = !!sym(fill_var))) +
    ...
    scale_fill_gradientn(
      colors = if (low_is_good) c("#4575B4", "#91BFDB", "#E0F3F8", "#FEE090", "#FC8D59")
                else c("#FC8D59", "#FEE090", "#E0F3F8", "#91BFDB", "#4575B4"),
...
create_median_days_plot <- function(data){
  ...
  median_day_color = tail(data$median_days_on_market, 1)
  #print("Flag:", median_day_color)
  ggplot(data, aes(x = month, y = median_days_on_market)) +
    geom_line(color = median_day_color, size = 1.5, alpha = 0.7) +
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5),
          ...
          axis.text = element_text(color = "snow3", size = 10),
          axis.title = element_text(size = 14),
          strip.background =element_rect(fill=median_day_color),
          strip.text = element_text(colour = '#272B30')) 
}
...
create_bar_plot <- function(data, var, var_name) {
  top_10 <- data %>%
    top_n(10, !!sym(var)) %>% # select top 10 values of var
    arrange(desc(!!sym(var)))  # sort in descending order by var

  p <- ggplot(top_10, aes(x = State, y = !!sym(var), fill = State)) +
    geom_bar(stat = "identity", show.legend = FALSE) +
    scale_y_continuous(limits = c(0, max(top_10[[var]], na.rm = TRUE) * 1.1), expand = c(0, 0)) +
    coord_flip() + # Flip the x and y axis
    scale_x_discrete(limits = top_10$State[order(top_10[[var]])]) +
...
server <- function(input, output, session) {
  ...
  # renderPlot context to render "Seasonal Plot of Median Days on the Market" plot
  ...
  # renderPlotly context to render bar plot for each select input variable using the look up table
  output$state_bar_plot <- renderPlotly({
    data_info <- data_lookup[[input$map_var]]
    
    if (input$map_var == "Weighted Sum") {
      join_data <- weighted_sum_data()
    } else {
      join_data <- data_info$join_data
    }
    
    create_bar_plot(join_data, data_info$var, input$map_var)
  })

  # renderText context to display the selected state's data
  output$selected_state_data <- renderText({
    if (!is.null(selected_state$state) && !is.null(selected_state$data)) {
      paste0("Selected State: ", str_to_title(selected_state$state), "\n", input$map_var, ": ", selected_state$data)
    }
  })

  # renderUI context to update the input drop down menu
  output$map_var_input <- renderUI({
    selectInput("map_var", "Choose Variable:",
                choices = c("Education Spending",
                            "# Citizens Per Hospital",
                            "Median Home Prices",
                            "Median Days on the Market",
                            "Weighted Sum"),
                selected = "Education Spending")
  })
}
